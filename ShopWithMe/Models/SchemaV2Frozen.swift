...
/// Eingefrorene Kopie des Datenmodell-Stands vor der Markenname-Erweiterung
/// an ``Geschaeft`` (GitHub #, v0.16) — repräsentiert exakt das Schema, das
/// auf bereits installierten Geräten bis zu diesem Zeitpunkt persistiert war.
/// Enthält gegenüber ``SchemaV1`` die in v0.13 eingeführten Modelle
/// ``Produkt``/``Produktname`` sowie die dazugehörigen Relationship-Ergänzungen
/// an ``Artikel``, ``Preispunkt`` und ``EinkaufslistenEintrag``.
///
...
///
/// **Nie mehr ändern:** Sobald `SchemaV3` (siehe `SchemaDefinition.swift`)
...
enum SchemaV2: VersionedSchema {
    ...
    final class Preispunkt {
        ...
        init(
            ...
            produktName: String?, alternativerName: String?, artikelNameSnapshot: String,
            geschaeftNameSnapshot: String, produkt: Produkt?
        ) {
            self.id = id
            self.artikel = artikel
            self.geschaeft = geschaeft
    ...
    final class EinkaufslistenEintrag {
        ...
        init(id: UUID, einkaufsliste: Einkaufsliste?, artikel: Artikel?, menge: Double, notiz: String?, erstelltAm: Date, produkt: Produkt?) {
            self.id = id
            self.einkaufsliste = einkaufsliste
            self.artikel = artikel
            self.menge = menge
    ...
    final class Produkt {
        ...
        init(id: UUID, name: String, artikel: Artikel?, elternProdukt: Produkt?, istStandard: Bool) {
            self.id = id
            self.name = name
            self.artikel = artikel
    ...
    final class Produktname {
        ...
        init(id: UUID, name: String, produkt: Produkt?, geschaeft: Geschaeft?) {
            self.id = id
            self.name = name
            self.produkt = produkt
            self.geschaeft = geschaeft
